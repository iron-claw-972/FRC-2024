package frc.robot.commands.autoComm;
...
public class PathPlannerCommand extends SequentialCommandGroup {
    ...
    public PathPlannerCommand(ArrayList<PathPoint> waypoints, swerveDriveImpl drive) {
        this(waypoints, drive, true);
    }

    public PathPlannerCommand(ArrayList<PathPoint> waypoints, swerveDriveImpl drive, boolean useAllianceColor) {
    ...
    public PathPlannerCommand(ArrayList<PathPoint> waypoints, swerveDriveImpl drive, boolean useAllianceColor, double maxSpeed, double maxAccel) {
        this(new ArrayList<PathPlannerTrajectory>(List.of(PathPlanner.generatePath(
    ...
    public PathPlannerCommand(String pathGroupName, int pathIndex, swerveDriveImpl drive) {
        this(PathGroupLoader.getPathGroup(pathGroupName), pathIndex, drive, true, true, false);
    }

    public PathPlannerCommand(String pathGroupName, int pathIndex, swerveDriveImpl drive, boolean resetPose) {
        this(PathGroupLoader.getPathGroup(pathGroupName), pathIndex, drive, resetPose, true, false);
    }

    public PathPlannerCommand(List<PathPlannerTrajectory> pathGroup, int pathIndex, swerveDriveImpl drive, boolean resetPose) {
        this(pathGroup, pathIndex, drive, resetPose, true, false);
    }

    public PathPlannerCommand(List<PathPlannerTrajectory> pathGroup, int pathIndex, swerveDriveImpl drive, boolean resetPose, boolean useAllianceColor, boolean isPerpetual) {
    ...
    public static PPSwerveControllerCommand createSwerveControllerCommand(
            ...
            Consumer<ChassisSpeeds> outputChassisSpeeds, boolean useAllianceColor, swerveDriveImpl drive, boolean isPerpetual) {
        if (isPerpetual) {
