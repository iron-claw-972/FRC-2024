...
public class TestSteerAngle extends CommandBase{
  ...
  public void execute() {
    m_drive.setModuleStates(new SwerveModuleState[] {
      new SwerveModuleState(0, new Rotation2d(m_drive.getRequestedSteerAngle(m_drive.modules[0].getAngle().getRadians()))),
      new SwerveModuleState(0, new Rotation2d(m_drive.getRequestedSteerAngle(m_drive.modules[1].getAngle().getRadians()))),
      new SwerveModuleState(0, new Rotation2d(m_drive.getRequestedSteerAngle(m_drive.modules[2].getAngle().getRadians()))),
      new SwerveModuleState(0, new Rotation2d(m_drive.getRequestedSteerAngle(m_drive.modules[3].getAngle().getRadians())))
    }, true);
